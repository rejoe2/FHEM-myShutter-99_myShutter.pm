...
min(@)
{
  ...
  Log 3, "min in 99_Utils called by $call_details[1] ($call_details[2]). Consider using minText (or minNum) instead!";
  return minText(@_);
}
...
{
  my @call_details = (caller(0));
  Log 3, "max in 99_Utils called by $call_details[1] ($call_details[2]). Consider using maxText (or maxNum) instead!";
  return maxText(@_);
}
