...
#
# $Id: 10_MYSENSORS_DEVICE.pm 18933 2019-03-17 06:33:39Z Beta-User $
#
##############################################
...
sub Set($@) {
    ...
    $command =~ m,(on|off), and do {
        if (defined my $stateReading = AttrVal($name, "mapStateReading",undef)) {
          readingsSingleUpdate($hash, 'state',"set $command", 1);
	   my $effect = $command =~ m,on, ? "on ": "off";
          AnalyzeCommandChain ($hash, "set $name $stateReading $effect");
        }
        last;
    };
    (defined ($hash->{setcommands}->{$command})) and do {
        ...
        if (defined my $stateReading = AttrVal($name, "mapStateReading",undef)) {
	   my $effect = ReadingsVal($name, "state",undef)) =~ s/set //;
          readingsSingleUpdate($hash, 'state',"$effect", 1);
        }
    };
...
sub onStreamMessage($$) {
    ...
    $type == ST_FIRMWARE_REQUEST and do {
        ...
        if (length($msg->{payload}) == 12) {
          ...
          readingsSingleUpdate($hash, "state", "updating", 1) unless (ReadingsVal($hash,"state","dead") eq "updating");
...
sub onInternalMessage($$) {
    ...
    $type == I_SKETCH_NAME and do {
        readingsSingleUpdate($hash, "state", "received presentation", 1);
...
sub timeoutAlive($) {
    my $hash = shift;
    Log3 $name, 5, "$hash->{NAME}: timeoutAlive called";
    readingsSingleUpdate($hash,"heartbeat","dead",1) unless (ReadingsVal($hash,"heartbeat","dead") eq "NACK");
}

sub timeoutAck($) {
    my $hash = shift;
    Log3 $name, 5, "$hash->{NAME}: timeoutAck called";
    if ($hash->{IODev}->{outstandingAck} == 0) {
      Log3 $name, 4, "$hash->{NAME}: timeoutAck called, no outstanding Acks at all";
      readingsSingleUpdate($hash,"heartbeat","alive",1) if (ReadingsVal($hash,"heartbeat","dead") eq "NACK");
       ...
       Log3 $name, 4, "$hash->{NAME}: timeoutAck called, outstanding: @$hash->{IODev}->{messagesForRadioId}->{$hash->{radioId}}->{messages}";
        readingsSingleUpdate($hash,"heartbeat","NACK",1) ;
    } else {
        Log3 $name, 4, "$hash->{NAME}: timeoutAck called, no outstanding Acks for Node";
        readingsSingleUpdate($hash,"heartbeat","alive",1) if (ReadingsVal($hash,"heartbeat","dead") eq "NACK");
    }
}
...
sub timeoutAwake($) {
    my $hash = shift;
    Log3 $name, 5, "$hash->{NAME}: timeoutAwake called";
