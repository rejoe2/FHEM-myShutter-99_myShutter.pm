# $Id: 98_WeekdayTimer.pm 23654 2021-02-01 06:28:26Z Beta-User $
#############################################################################
#
#     98_WeekdayTimer.pm
...
package FHEM::WeekdayTimer;  ## no critic qw( package )
...
sub WeekdayTimer_SetTimer {
  ...
  for(0..$#switches) {
    ...
    next if !$timToSwitch - $now > -5 && !defined $hash->{SETTIMERATMIDNIGHT};
  ...
  return if !$switchInThePast || !defined $aktTime || checkDelayedExecution($hash, $aktParameter, $aktIdx );
    ...
    Log3( $hash, 4, "[$name] past timer on $device at ". FmtDateTime($aktTime). " with  $aktParameter activated" );
