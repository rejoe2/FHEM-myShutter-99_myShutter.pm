...
<b>Important Note:</b>  
  ...
  You may also need copy an older version of WeedayTimer to your module path to get this to work! It's highly recommended to just convert all Heating_Control devices to WeedayTimer by either manually edit your cfg file and replace just the module in define statement and add a reasonable WDT_Group attribute, e.g. "former_HC" - this is more or less the functionality, ConvertToWDT offers... <br>
  To easen life, just change/import the relevant parts of your config by using the multiline command line option FHEMWEB offers.
  <br><br>
